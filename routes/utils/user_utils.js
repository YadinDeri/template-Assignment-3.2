...
async function markAsFavorite(user_id, recipe_id) {
    ...
    const prevList = JSON.stringify(existingFavorites[0].recipe_ids)
    console.log(existingFavorites[0].recipe_ids)
    console.log("pushhhh")
    ...
    // const newList =
    //existingFavorites.recipe_ids = recipe_id

    // Update the FavoriteRecipes table
    //await DButils.execQuery(`UPDATE FavoriteRecipes SET recipe_ids ='${existingFavorites}' WHERE user_id='${user_id}'`);
}
...
async function getFavoriteRecipes(user_id){
    const recipes_id = await DButils.execQuery(`select recipe_ids from FavoriteRecipes where user_id='${user_id}'`);
    return recipes_id;
}
//------------------------------
